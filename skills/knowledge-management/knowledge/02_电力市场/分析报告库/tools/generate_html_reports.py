#!/usr/bin/env python3
"""批量将 markdown 报告转换为带样式的 HTML 页面并更新 index.html"""
import os, re
...
WS_DIR = os.path.expanduser("~/openclaw/workspace")
...
def md_to_html(content):
    ...
    while i < len(lines):
        ...
        if in_code:
            ...
            continue
        if stripped.startswith('#### '):
            flush_list(); html_lines.append(f'<h4>{_inline(stripped[5:])}</h4>')
        elif stripped.startswith('### '):
            flush_list(); html_lines.append(f'<h3>{_inline(stripped[4:])}</h3>')
        elif stripped.startswith('## '):
            flush_list(); html_lines.append(f'<h2>{_inline(stripped[3:])}</h2>')
        elif stripped.startswith('# '):
            pass
        elif stripped.startswith('|'):
            ...
            flush_list(); html_lines.append(_make_table(table_lines)); table_lines = []; in_table = False; continue
        elif re.match(r'^---+$', stripped):
            flush_list(); html_lines.append('<hr>')
        elif stripped.startswith('- ') or stripped.startswith('* '):
            if not in_list or list_type != 'ul': flush_list(); html_lines.append('<ul>'); in_list = True; list_type = 'ul'
            html_lines.append(f'<li>{_inline(stripped[2:])}</li>')
        elif re.match(r'^\d+\.\s+', stripped):
            if not in_list or list_type != 'ol': flush_list(); html_lines.append('<ol>'); in_list = True; list_type = 'ol'
            html_lines.append(f'<li>{_inline(re.sub(r"^\d+\.\s+", "", stripped))}</li>')
        else:
            flush_list()
            if stripped: html_lines.append(f'<p>{_inline(stripped)}</p>')
            else: html_lines.append('')
        i += 1
    flush_list()
    if in_table: html_lines.append(_make_table(table_lines))
